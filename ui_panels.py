...
class GP_PT_sidebarPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        props = col.operator('gp.latticedeform', icon ="MOD_MESHDEFORM")# MOD_LATTICE, LATTICE_DATA
        
        # Keep lattice options
        box = self.layout.box()
        row = box.row(align = True)

        row.prop(props, "keep_lattice", text = "Keep lattice")
        row.prop(props, "create_hooks", text = "Use Hooks")
        
        col = self.layout.column(align=True, heading="Stroke")
                
        # Straight line ops
        col.operator('gp.straight_stroke', icon ="CURVE_PATH")# IPO_LINEAR

        col = self.layout.column(align=True, heading="View")

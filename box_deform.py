...
"""
add_hook_to_lattice
params: lattice -> lattice to add hooks to
        gp_name -> the grease pencil name that the lattice is being applied to
this method adds an empty and hook modifier for each set of vertices with a unique
(x, z) location, duplicate y positions are controled by the same hook.
"""
def add_hooks_to_lattice(lattice: bpy.types.Lattice, gp_name: str):
    points = lattice.data.points
    
    matrix_world = lattice.matrix_world 
    
    y_hooks = {}
    
    for idx, p in enumerate(points):
        
        #there is probably a better way to discrimnate the unique x/z pairs
        #but I went with using pow
        if pow(p.co[0], p.co[2]) not in y_hooks.keys():
            #found a unique x/z pair, create a new hook modifier
            hook = lattice.modifiers.new(name='hook', type="HOOK")
            
            #assign the hook modifer to the index of the vertex
            hook.vertex_indices_set([idx])
            
            #create a new empty
            bpy.ops.object.empty_add(type='PLAIN_AXES', align='WORLD', location=(p.co @ matrix_world), scale=(1, 1, 1))
            
            empty = bpy.context.active_object
            
            empty.name = gp_name + ".Hook"
            
            #assign the empty to the hook modifier
            hook.object = empty
            
            #track the hook modifier to the unique x/z pair
            y_hooks[pow(p.co[0], p.co[2])] = hook
        else:
            #this is not a unique x/z pair, so retreive the hook modifier
            hook = y_hooks[pow(p.co[0], p.co[2])] 
            
            i = []
            
            #what is a bpy_prop_array? why can I not treat like a list?
            for v in range(len(hook.vertex_indices)):
                i.append(hook.vertex_indices[v])
...
def view_cage(obj, options):
    ...
    if cage:
        if options["keep_lattice"]:
            
            cage.name = options["gp_name"] + ".Lattice"
            if options["create_hooks"]:
                #add hooks to lattice                                
                add_hooks_to_lattice(cage, options["gp_name"])                
        else:
...
class GP_OT_latticeGpDeform(bpy.types.Operator):
    ...
    keep_lattice: bpy.props.BoolProperty(name="Keep Lattice", default=True)
    create_hooks: bpy.props.BoolProperty(name="Create Hooks", default=True)
...
valid:Spacebar/Enter, cancel:Del/Backspace/Tab/Ctrl+T"
    ...
    def invoke(self, context, event):
        ...
        self.cage = view_cage(self.gp_obj, {"keep_lattice" : self.keep_lattice, "create_hooks" : self.create_hooks, "gp_name" : self.gp_obj.name})
